...
function logDictDiff() {
  Object.entries(loadItems()).forEach(([k,v])=> {
    if (unsafeWindow.words.items[k] === v) return;
    if (unsafeWindow.words.artifact[k] === v) return;
    console.log(k, v, unsafeWindow.words.items[k])
  });
  Object.entries(loadEquips()).forEach(([k,v])=> {
    v = v.replaceAll(/<\/*span.*?>/g,'')
    if (unsafeWindow.words.equipsName[k] === v) return;
    if (unsafeWindow.words.equipsPart[k] === v) return;
    if (unsafeWindow.words.equipsInfo[k] === v) return;
    if (unsafeWindow.words.character[k] === v) return;

    console.log(k, v, unsafeWindow.words.equipsName[k], unsafeWindow.words.equipsPart[k], unsafeWindow.words.equipsInfo[k])
  });
}
...
function loadEquips(){
    ...
    var equips = {
        ...
        ////////////////////////材质前缀////////////////////////
        //布甲
        'Cotton':'棉制<span style=\"background:#FFFFFF;color:#000000\" >(布)</span>',
        ...
        'Frugal':'<span style=\"color:red\" >节约的(省魔)</span>',
        ...
        'Reinforced':'坚固的(减伤)',
        'Shielding':'盾化的(格挡)',
        'Arctic':'<span style=\"background:#94c2f5\" >北极(冰)</span>',
        'Fiery':'<span style=\"background:#f97c7c\" >红莲(火)</span>',
        'Shocking':'<span style=\"background:#f4f375\" >雷鸣(雷)</span>',
        'Tempestuous':'<span style=\"background:#7ff97c\" >风暴(风)</span>',
        ...
        /////////////////品质//////////
        'Flimsy' : '薄弱',
        'Crude':'<span style=\"background:#acacac\" >劣等</span>',
