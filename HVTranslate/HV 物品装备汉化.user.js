...
function main(){
    ...
    translateOnLoaded('.eqshop_pane>div>div>#item_pane,#inv_equip,.eqshop_pane>div>div>#shop_pane,#inv_eqstor', 'hvut-eq-loading');
    translateOnChange('#popup_box');
    translateOnLoaded('.hvut-bt-equip>li', '获取附魔状态中');
    ...
    translateOnChange('.hvut-bt-inventory', translateItems);
...
function translate(target, dicts) {
    ...
    for (var dict of dicts){ //遍历字典并翻译
        temp = temp.replace(dict[0], dict[1]);
    }
    if (isElem) {
        if (html !== temp) {
...
function translateOnLoaded(selector, loadingText, method = undefined, waitNext = false) {
  ...
  if (loadingText) { // Type 1 : 通过加载中的文本判断
    ...
    for (let i = 0; i < all.length; i++) {
      if (all[i].innerHTML.includes(loadingText)) {
        setTimeout(() => translateOnLoaded(selector, loadingText, method), 100);
        return;
      }
    }
    method(selector);
  ...
  Array.from(document.querySelectorAll(selector)).forEach(elem => observer.observe(elem, { childList: true, attribute: true, attributeFilter: ['value', 'title'] })); //监听翻译动态内容
}

function translateOnChange(selector, method = undefined) {
  method = method ?? translateEquips;
  const observer = new MutationObserver((mutations, observer) => {
    if (!translated) return;
    mutations.forEach(mutation => {
      ...
      nodes.forEach(node => {
        ...
        method(selector);
        node => observer.observe(node, { childList: true, attribute: true, attributeFilter: ['value', 'title'] });
      })
    });
  });
  method(selector);
...
function loadItems(){
    ...
    var items = {
        ...
        'Divine Magic Proficiency' : '(神授熟练)',
        'Forbidden Magic Proficiency' : '(禁忌熟练)',
        'Deprecating Magic Proficiency' : '(衰折熟练)',
...
function loadExtra() {
    var dict = {
        ...
        'Heavy': '重',
        //论坛里的彩虹无双品质名
        'P(<[^<]+>)+e(<[^<]+>)+e(<[^<]+>)+r(<[^<]+>)+l(<[^<]+>)+e(<[^<]+>)+s(<[^<]+>)+s': '<span style=\"background:#ffd760;color:black\" >☯无双☯</span>',
